...
![NFET Characteristics](Images/nfet_01v8..png)<br>
...
![Id vs Vgs](Images/NMOS%20id%20vs.png)<br>
...
![Id vs Vds](Images/NMOS%20id%20vs%20Vds.png)
...
![pMOS inverter](Images/Pmos_inv%20(2).png)<br><br>
![pMOS inverter Transient](Images/Pmos%20inv%20wave.png)<br>
...
![CMOS inverter VTC](Images/vtc_char_cmos.jpg)
...
![CMOS inverter VTC](Images/CMOS_nseDC.png)
...
![CMOS inverter tran](Images/CMOS_DLY%20anlys.png)<br>
...
The above rise and fall time analysis is for Wp = 1.58 and Wn = 0.45. Also it is defined for the transition when output goes from 90% to 10% for fall time, and from 10% to 90% transition for rise time. The results are **trise = 108.505ps** and t**fall = 69.12ps**<br>
...
2. **Increase width of transistors**: Increasing transistor widths could increase their drive strengths decreasing rise and fall time because it would reduce Ron. However, tradeoff here is area. So I made Wp = 2.08 and Wn = 0.90 and run the simulations:
...
In the above loaded analysis where I have taken Wp = 1.58 and Wn = 0.45, we get **trise = 689.73ps** and t**fall = 824.49ps**<br><br>

![rise and fall time](Images/inv_wp=2.08_wn=0.90.png)<br>
In another loaded analysis  I have taken Wp = 2.08 and Wn = 0.90, we get **trise = 542.27ps** and **tfall = 419.40ps**<br><br>
...
![power](Images/pwr_anly.png)<br>
To characterize the power of the Sky130 inverter (Wp =1.58um,Wn=0.45um), I performed a transient analysis and calculated an average power dissipation of 42.945uW by integrating the supply current over a stable switching period (6.6ns to 13.2ns). Since the instantaneous current is very small, I applied a 1000x scaling factor to the power plot, which successfully visualized the impulsive switching "spikes" that represent the dynamic charging of the 0.1pF load and internal short-circuit energy. The resulting waveform confirms that the inverter consumes energy only during logic transitions, maintaining near-zero static power in accordance with CMOS theory.
...
<img src="Images/CMOS_INV%20LAY.png" width="70%" height="70%" alt="CMOS Inverter Layout"></img><br>
